<template>
  <div class="auth-container">
    <div class="auth-box" v-if="!currentUser">
      <h2 class="auth-heading">{{ hange ? "Register" : "Login" }}</h2>
      <button class="auth-toggle-btn" @click="change">
        {{ hange ? "Login" : "Register" }}
      </button>

      <form class="auth-form" @submit.prevent="register" v-if="hange">
        <input
          class="auth-input"
      ...
      <form class="auth-form" v-else @submit.prevent="sign">
        <input
          class="auth-input"
          placeholder="Email address..."
          v-model="email"
